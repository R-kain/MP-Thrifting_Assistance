...
namespace
{
    ...
    {
        const std::size_t seed = std::hash<std::string>{}(imagePath.filename().string());
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Current analysis values are placeholder metrics until the OpenCV detector is wired in.\n";
    ...
    {
        std::cout << "OpenCV headers/libs are not configured in this build, so overlay windows are skipped.\n";
    }
    else
    {
        std::cout << "OpenCV overlay windows are enabled. Detected issue regions will be drawn on the source image.\n";
    }
    ...
    for (const auto& imagePath : imageFiles)
    {
        ...
        if (visualizer.IsAvailable() && !visualizer.Show(imagePath, metrics, report))
        {
            std::cerr << "Failed to open OpenCV overlay window for: " << imagePath.string() << "\n";
        }
    }
